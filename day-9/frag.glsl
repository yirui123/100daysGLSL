...
const int ambienOcclusionSteps = 30;
const float PI = 3.14159;
vec2 field( vec3 position )
{

    vec2 sph = sphere( position, 25. );
    ...
    vec2 cyl = cylinder( position, 10.,20., vec3( 0.,-1.5,0.) );
...
vec3 rimlight( vec3 pos, vec3 nor )
{
    ...
    float vdn = 1.5 - max(dot(v, nor), 0.0);
    return vec3(smoothstep(0., 1.5, vdn));
}
void main() {


        vec3 color = vec3( 0.4*sin(time), 0.8, 0.99 );    //blue
       vec2  screenPos    = squareFrame( resolution );
       float col = pow( max( 0., 1.- length( screenPos - vec2( 0.,.35 ) ) ), 10.5 );
       gl_FragColor = vec4( vec3( col ), 1. );


       float cameraAngle   = 0.2;
       float cameraRadius  = 30.+sin( time*.1);

       float lensLength   = .25  * time ;
